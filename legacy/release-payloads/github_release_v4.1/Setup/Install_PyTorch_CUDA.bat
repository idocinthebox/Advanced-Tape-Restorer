...
if %PY_MAJOR% equ 3 if %PY_MINOR% lss 9 (
    ...
    echo (Python 3.12 is required for VapourSynth R73 installer compatibility)
    pause
    exit /b 1
)
...
REM Use goto instead of if blocks to avoid batch file issues
if "%CUDA_AVAILABLE%"=="1" goto :install_cuda
goto :install_cpu

:install_cuda
echo Installing with CUDA 12.6 support for GPU acceleration (Blackwell/Ada/Ampere)...
echo This may take 10-15 minutes...
echo.
%PYTHON_CMD% -m pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu126
goto :after_install
...
if %PIP_ERROR% neq 0 (
    ...
    echo   Select: Stable, Windows, Pip, Python, CUDA 12.6
    echo.
    pause
    exit /b 1
)
...
if %errorLevel% equ 0 goto :pytorch_cuda_success
goto :pytorch_no_cuda
...
echo.
if "%CUDA_AVAILABLE%"=="1" goto :show_cuda_warning
...
echo   - Driver version too old for CUDA 12.6 (requires 545.84+)
...
REM Check if Python 3.12 is already installed via Python Launcher
echo Checking for Python 3.12 installation...
py -3.12 --version >nul 2>&1
if %errorLevel% equ 0 (
    echo [OK] Python 3.12 found via Python Launcher
    echo Using py -3.12 for PyTorch installation
    set PYTHON_CMD=py -3.12
    goto :gpu_check
)

echo Would you like to automatically install Python 3.12 alongside Python %PY_VERSION%?
echo (Both versions can coexist - you can use py -3.12 to access it)
echo (Python 3.12 is also required for VapourSynth R73 installer)
echo.
choice /C YN /M "Install Python 3.12 for compatibility"
if %errorLevel% equ 2 (
    echo.
    echo Installation cancelled.
    echo.
    echo To install manually:
    echo   1. Download Python 3.12.8: https://www.python.org/ftp/python/3.12.8/python-3.12.8-amd64.exe
    echo   2. During install, check "Add Python to PATH"
    echo   3. Use "py -3.12" command to run Python 3.12
    echo   4. Run: py -3.12 -m pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu126
    echo.
    pause
    exit /b 1
)
...
powershell -Command "& { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $webClient = New-Object System.Net.WebClient; $webClient.DownloadFile('%PYTHON_URL%', '%INSTALLER%'); $webClient.Dispose(); }"
...
echo.
start /wait "" "%INSTALLER%" /passive PrependPath=1 Include_test=0
...
echo.
echo Continuing installation with Python 3.12...
set PYTHON_CMD=py -3.12
echo.
goto :gpu_check

:python_stub_detected
echo [ERROR] Python not properly installed (Windows Store stub detected)
echo.
echo Please disable Windows Store aliases:
...
echo   - Re-run this script for automatic installation
echo.
pause
exit /b 1
