...
class ModelConverter:
    ...
    def load_pytorch_model(self, model_name: str) -> Optional[torch.nn.Module]:
        ...
        try:
            ...
            print(f"[X] Failed to load {model_name}: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _load_demo_model(self, model_type: str) -> Optional[torch.nn.Module]:
        ...
        try:
            ...
            print(f"[X] Could not import demo_model: {e}")
            return None
        ...
        try:
            ...
            print(f"   [OK] Demo {model_type} loaded ({total_params:,} parameters) on {device}")
            return model
        except Exception as e:
            print(f"[X] Failed to create demo model: {e}")
            return None
    ...
    def _load_gfpgan(self) -> Optional[torch.nn.Module]:
        ...
        try:
            ...
            print(f"[X] GFPGAN not installed: {e}")
            print("   Install with:")
        ...
        if not model_files:
            print(f"[X] No GFPGAN model found in: {model_dir}")
        ...
        if hasattr(restorer, 'gfpgan'):
            ...
            print(f"   [OK] GFPGAN model loaded on {device}")
            return model
        else:
            print(f"[X] Could not extract model from GFPGANer")
            return None
    ...
    def _load_realesrgan(self) -> Optional[torch.nn.Module]:
        ...
        if not model_files:
            print(f"[X] No RealESRGAN model found in: {model_dir}")
            return None
        ...
        try:
            ...
            model.eval()
            model.to(device)
            print(f"   [OK] RealESRGAN model loaded on {device}")
            return model
        except Exception as e:
            print(f"[X] Failed to load model weights: {e}")
    ...
    def _load_rife(self) -> Optional[torch.nn.Module]:
        ...
        if not model_files:
            print(f"[X] No RIFE model found in: {model_dir}")
            return None
        ...
        model.eval()
        model.to(device)
        print(f"   [OK] RIFE model loaded on {device}")
        return model
    ...
    def _load_basicvsrpp(self) -> Optional[torch.nn.Module]:
        ...
        if not model_files:
            print(f"[X] No BasicVSR++ model found in: {model_dir}")
            return None
        ...
        model.eval()
        model.to(device)
        print(f"   [OK] BasicVSR++ model loaded on {device}")
        return model
    ...
    def _load_swinir(self) -> Optional[torch.nn.Module]:
        ...
        if not model_files:
            print(f"[X] No SwinIR model found in: {model_dir}")
            return None
        ...
        model.eval()
        model.to(device)
        print(f"   [OK] SwinIR model loaded on {device}")
        return model
    ...
    def _load_deoldify(self) -> Optional[torch.nn.Module]:
        ...
        if not model_files:
            print(f"[X] No DeOldify model found in: {model_dir}")
            return None
        ...
        model.eval()
        model.to(device)
        print(f"   [OK] DeOldify model loaded on {device}")
        return model
    ...
    def verify_existing(self):
        ...
        if not onnx_dir.exists():
            print("[X] No ONNX models directory found")
